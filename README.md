...
---
## 📢 V2.6 版本更新重点
v2.6 @Mufanc 大佬下场，彻底优化了底层操控手机的实现，彻底解决了多指触控等不稳定问题。

## 📢 V2.5 版本更新重点
v2.5 增加用户自定义命令任务，并且可以选择跳过复位逻辑，方便社区共建各种活动脚本，大家可以由创建者指定方式运行

v2.5.1 优化了作物状态识别，现在支持自定义截图位置和提示词，优化了默认提示词。底层自动合并缺失的新参数了。
...
📺 **[点此播放视频 教程 v2.5新功能部分](https://www.bilibili.com/video/BV1f3BXBuEYX/)**
...
### 7. 💰 智能售卖
* **直接卖小麦等**：支持自定义库存线，自动通过 OCR 识别当前数量，精准计算并售卖溢出部分。

### 8. 🧩 自定义扩展
* **社区共建**：开放标准化的指令接口，支持导入用户编写的特定活动脚本（如限时任务），实现逻辑的快速分享与复用。
* **弹性执行**：打破强制初始化的限制，支持配置“跳过复位”模式。脚本可基于当前 UI 状态直接介入，由创作者指定运行环境，实现复杂场景下的即插即用。
...
### Q1:选择了一个正在生长的作物，询问AI，AI返回plant，然后开始播种操作?

A: 可能屏幕中有工厂有待收获的植物样子的东西（例如酸菜），建议把这些工厂移出截屏范围
